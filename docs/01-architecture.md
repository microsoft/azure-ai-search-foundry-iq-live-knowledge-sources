...
| Native Knowledge Base MCP endpoint | [`src/liveks/mcp_client.py`](https://github.com/microsoft/azure-ai-search-foundry-iq-live-knowledge-sources/blob/main/src/liveks/mcp_client.py), [`samples/python/knowledge_base_mcp_consumer.py`](https://github.com/microsoft/azure-ai-search-foundry-iq-live-knowledge-sources/blob/main/samples/python/knowledge_base_mcp_consumer.py), and [Knowledge Base MCP Client](22-knowledge-base-mcp.md) | The independent consumer must pass endpoint, authentication, `tools/list`, `tools/call`, text-content, and known-fact checks. `liveks mcp --expect-term <known-fact>` retains the lifecycle-bound equivalent. | Read/call only. Pair MCP content with the source-specific retrieve trace before naming the source that ran. |
...
## Provider Operations And Lifecycle Policy

The direct Search data-plane profiles use a deliberately small internal boundary. [`src/liveks/providers/sources.py`](https://github.com/microsoft/azure-ai-search-foundry-iq-live-knowledge-sources/blob/main/src/liveks/providers/sources.py) builds and checks the demonstrated Search Index, MCP Server, native Fabric Ontology, and Knowledge Base definitions. [`src/liveks/providers/data_plane.py`](https://github.com/microsoft/azure-ai-search-foundry-iq-live-knowledge-sources/blob/main/src/liveks/providers/data_plane.py) normalizes read, conditional create, retrieve, and conditional delete requests. Stable and preview API versions remain explicit on every operation.

`src/liveks/cli.py` remains the lifecycle authority. It selects profiles, orders independent source checks before combined retrieval, classifies public failures, journals locks and ETags, authorizes cleanup, preserves BYO assets, and emits allowlist-only evidence. Provider modules cannot decide ownership or delete without an ETag supplied by the lifecycle. Provisioned profiles keep their existing script boundary. This is an internal test seam for current implementations, not a plugin system or third-party provider API.

The northbound MCP protocol helpers in `src/liveks/mcp_client.py` are lifecycle-neutral. Both the independent sample and `liveks mcp` use the same request and content parsing contract, while endpoint discovery, Azure CLI credential acquisition, environment reports, and source-readiness sequencing remain in `src/liveks/cli.py`.
