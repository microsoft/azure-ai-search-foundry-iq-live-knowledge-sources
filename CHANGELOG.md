...
- A lifecycle-independent Python consumer for the native Knowledge Base MCP endpoint, with environment-only bearer/admin-key inputs, JSON/SSE `tools/list` and `tools/call`, mandatory known-fact acceptance, bounded transient retries, deterministic exit codes, normalized failures, and allowlist-only evidence. Credential-free tests prove transport and LiveKS protocol parity; MCP/Azure/Fabric live execution is **NOT RUN**.
